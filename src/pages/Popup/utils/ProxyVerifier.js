...
var ProxyVerifier = (module.exports = {
  ...
  testAll: function (proxy, options, cb) {
    ...
    ProxyVerifier.testProtocols(
      proxy,
      testProtocolsOptions,
      function (error, protocolsResult) {
        if (error) {
          return cb(error);
        }

        var workingProtocols = _.filter(
          _.keys(protocolsResult),
          function (protocol) {
        ...
        async.parallel(asyncTests, function (error, results) {
          ...
          results.protocols = _.object(
            _.map(_.keys(protocolsResult), function (protocol) {
              return [
                protocol,
                _.omit(protocolsResult[protocol], 'data', 'status', 'headers'),
              ];
            })
          );
  ...
  testProtocols: function (proxy, options, cb) {
    ...
    var tests = _.object(
      _.map(proxy.protocols, function (protocol) {
        var _proxy = _.extend({}, proxy, { protocol: protocol });
        return [
          protocol,
          _.bind(ProxyVerifier.testProtocol, undefined, _proxy, options),
        ];
      })
    );
  ...
  testAnonymityLevel: function (proxy, options, cb) {
    ...
    async.parallel(
      ...
      function (error, results) {
        ...
        var ipAddressMatches =
          !!myIpAddress && withProxy.data.ip_addr === myIpAddress;
        var ipAddressRegEx = /[:.0-9a-z%]+/g;
        var anyHeaderContainsIpAddress = !!_.chain(withProxy.data)
        ...
        if (ipAddressMatches || anyHeaderContainsIpAddress) {
          ...
          var hasProxyHeaders = _.some(proxyHeaders, function (proxyHeader) {
            return (
              _.contains(headerKeys, proxyHeader) ||
              _.some(headerKeys, function (headerKey) {
  ...
  getMyIpAddress: function (options, cb) {
    ...
    options = _.defaults(options || {}, {
      ...
      ipAddressCheckFn:
        ProxyVerifier._getIpAddressFromCheckProxyServiceResponse,
    });

    ProxyVerifier.request(
      'get',
      options.ipAddressCheckUrl,
      options,
  ...
  test: function (proxy, options, cb) {
    ...
    async.until(
      function (next) {
        ...
        var doContinue =
          (result && result.ok === true) || numAttempts >= options.maxAttempts;
      ...
      function (nextAttempt) {
        ...
        ProxyVerifier.request(
          ...
          function (error, data, status, headers) {
            ...
            setTimeout(
              nextAttempt,
              result.ok === true ? 0 : options.waitTimeBetweenAttempts
            );
          }
        );
      },
  ...
  normalizeProxy: function (proxy) {
    ...
    return {
      ...
      protocols:
        proxy.protocols || (proxy.protocol && [proxy.protocol]) || null,
...
ProxyVerifier.loadCountryData = deprecate.function(function (options, cb) {
  // Do nothing.
  _.delay(cb);
}, 'loadCountryData() has been deprecated and will be removed in a future release');

ProxyVerifier.loadCountryDataSync = deprecate.function(function (options) {
  // Do nothing.
